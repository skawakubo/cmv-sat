...
# Usage: Usage: python select_alignment_site.py <input file (aligned fasta)> <output file prefix> <minimum site coverage> <minimum proportion of the non-gapped sites>
...
for name in seq_d:
    ...
    if aligned_seq_len > thresh_aligned_seq_len:
        print(">" + name + "\n" + seq_without_gap, file=seq_all_pos_out_f)
        print(">" + name + "\n" + seq_selected_pos, file=seq_selected_pos_out_f)
